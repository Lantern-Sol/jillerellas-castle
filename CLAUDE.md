...
11. **Dynamic sources in JSON templates take no filters.** `{{ closest.product.description | strip_html }}` is rejected at upload ("metafield filter … is not supported"), and two filters are rejected outright ("must not use more than 1 filter"). Shape the value in CSS or in the block, not in the template.
12. **Range settings need 3–101 steps.** `(max - min) / step + 1` outside that range fails at upload with "step invalid". A 2-option range must be a `select`.
13. **At ≥990px `.page-wrapper` is the scroller, not the document** ([assets/base.css](assets/base.css#L36)). `window.scrollTo` / `scrollIntoView` from devtools do nothing on desktop, so lazily-fetched sections (recommendations) never enter the viewport. Scroll `.page-wrapper`. Below 990px the document scrolls normally.
14. **`text` blocks accept a `custom_class`** setting — the hook for page CSS that stock settings can't express. Added to [blocks/text.liquid](blocks/text.liquid); `snippets/text.liquid` already took the `class` param.
15. The Figma product card is shared by the collection grid, "Pairs well with" and "You may also like": its rules live once in [assets/collection-page.css](assets/collection-page.css) under `.product-card-grid`. Add that class rather than copying the rules.
16. **Two upstream rules that fight the card design** (both reset in [assets/product-page.css](assets/product-page.css)): `base.css:5209` borders *every direct child* of `.product-recommendations`, drawing dark boxes around the section and card row; and `slideshow-slide` sets `overflow: hidden`, which shears card drop-shadows off inside any carousel. Padding the scroll track to recover the shadow needs three companions or it causes worse bugs — negative margin (position), `width: calc(100% + 2×pad)` (the track is border-box, and slide widths are a share of its content box), and `scroll-padding-inline` (snapping aligns to the padding box, so the row rests scrolled).
16b. **Carousel slides come out narrow when pagination dots are on.** `--peek-next-slide-size` (3rem) is subtracted from the track before the per-column width is divided out; with dots there is no peek, so the row ends short of the content column and the leftover strands on one side. Zero it (plus `--mobile-card-size`) as [snippets/resource-list.liquid](snippets/resource-list.liquid) and [snippets/resource-list-carousel.liquid](snippets/resource-list-carousel.liquid) both now do.
17. **Custom SVG icons need `fill: currentColor`.** Paths in `snippets/icon.liquid` mostly carry no `fill`, so they render black; the stock icon block only works because base.css styles `.icon-default`.
